#!/usr/bin/env python3
"""
Test B2B Situational Betting Strategy (FIXED)

Fixed spread calculation - the spread column contains absolute values,
and whos_favored indicates which team is favored.
"""
...
print("=" * 80)
print("B2B SITUATIONAL BETTING STRATEGY BACKTEST (FIXED)")
print("=" * 80)
...
# FIX: Apply spread sign based on whos_favored
# Spread is stored as absolute value, whos_favored tells us which team is favored
# Convention: negative spread = home favored, positive spread = away favored
odds['spread_signed'] = odds.apply(
...
merged = games.merge(
    odds[['date', 'home_team', 'away_team', 'spread_signed', 'total', 'whos_favored']],
    on=['date', 'home_team', 'away_team'],
    how='left'
).sort_values('date').reset_index(drop=True)

print(f"Merged to {len(merged)} games with odds")
print(f"Games with spread data: {merged['spread_signed'].notna().sum()} ({merged['spread_signed'].notna().mean():.1%})")
...
test_data = test_data.dropna(subset=['spread_signed'])
...
# If home is -5, they need to win by MORE than 5 to cover
# spread_result = home_margin + spread
# If home wins by 7 and spread is -5: 7 + (-5) = 2 (home covers)
# If home wins by 3 and spread is -5: 3 + (-5) = -2 (home doesn't cover)
test_data['home_margin'] = test_data['home_score'] - test_data['away_score']
test_data['spread_result'] = test_data['home_margin'] + test_data['spread_signed']
...
print(f"\nSpread distribution:")
print(f"  Negative spreads (home favored): {(test_data['spread_signed'] < 0).sum()} ({(test_data['spread_signed'] < 0).mean():.1%})")
print(f"  Positive spreads (away favored): {(test_data['spread_signed'] > 0).sum()} ({(test_data['spread_signed'] > 0).mean():.1%})")
print(f"  Pick'em (spread = 0): {(test_data['spread_signed'] == 0).sum()}")

print(f"\nOverall cover rates:")
print(f"  Home covers: {test_data['home_covers'].mean():.1%}")
print(f"  Away covers: {test_data['away_covers'].mean():.1%}")
print(f"  Pushes: {test_data['push'].mean():.1%}")

if abs(test_data['home_covers'].mean() - 0.50) > 0.05:
    print(f"\n  ⚠️ WARNING: Home cover rate is {test_data['home_covers'].mean():.1%}, expected ~50%")
else:
    print(f"\n  ✅ Cover rates look correct (close to 50/50)")
...
# Strategy 3: Bet on team with rest advantage
print("\n--- REST ADVANTAGE ---")
...
# Strategy 5: Extreme rest disadvantage
print("\n--- EXTREME FATIGUE ---")
...
if len(results_df) > 0:
    # Filter profitable strategies with n >= 50
    profitable = results_df[(results_df['n'] >= 50) & (results_df['roi'] > 0)].sort_values('roi', ascending=False)

    if len(profitable) > 0:
        print("\nPROFITABLE strategies with n >= 50:")
        ...
        print("\nNo profitable B2B strategies found with n >= 50")
...
print("=" * 80)
baseline_home = backtest_strategy(test_data, "Bet HOME (all games)", test_data['spread_signed'].notna(), bet_home=True)
baseline_away = backtest_strategy(test_data, "Bet AWAY (all games)", test_data['spread_signed'].notna(), bet_home=False)
